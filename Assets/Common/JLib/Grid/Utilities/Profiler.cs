    ...
    public class Profiler
    {
        ...
        private void AddInternal(string name)
        {
            ...
            while (i < NUM_PROFILE_SAMPLES && Samples[i].Valid == true)
            {
                if (Samples[i].Name == name)
                {
                    ...
                    Debug.Assert(Samples[i].OpenProfiles == 1); //max 1 open at once
                    return;
                }
                i++;
            }
            ...
            {
                Debug.Assert(false);  // too many samples
                return;
            }
        ...
        private void DumpInternal()
        {
            ...
            if (StartProfile != EndProfile)
            {
                ...
                Debug.Print("  Ave :   Min :   Max :   # : Profile Name");
                Debug.Print("--------------------------------------------");
                ...
                while (i < NUM_PROFILE_SAMPLES && Samples[i].Valid == true)
                {
                    ...
                    {
                        Debug.Assert(false);    //"ProfileEnd() called without a ProfileBegin()" 	
                    }
                    else if (Samples[i].OpenProfiles > 0)
                    {
                        Debug.Assert(false); // ( !"ProfileBegin() called without a ProfileEnd()" );
                    }

                    Debug.Assert(EndProfile != StartProfile);
        ...
        void StoreProfileInHistory(string name, float percent)
        {
            ...
            {
                Debug.Assert(false);    //Exceeded Max Available Profile Samples!
            }
        }
