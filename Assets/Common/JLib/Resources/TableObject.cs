/*
 * This is a factory system to build objects from csv table entries
 */
using System;
...
namespace JLib.Utilities
{
        ...
        where Tbuilder : class, new()
    {
        ...
        public virtual void LoadFromAsset(string assetName)
        {
            LoadFromData(FileUtils.ReadFromAsset(assetName));
        }

        public virtual void LoadFromData(string rawData)
        {
        ...
        static void AddValues(object obj, Dictionary<string, object> data)
        {
            ...
            foreach (FieldInfo info in obj.GetType().GetFields())
            {
                ...
                {
                    Dbg.LogWarning("Failed to fill field " + info.Name);
                }
            }
        }
    ...
    public static class ExcelObjectReader
    {
        ...
        {
            Dbg.Assert(System.IO.File.Exists(filename));
        ...
        static void AddValue(object obj, DataColumn col, object sourceObject)
        {
            ...
            if (info == null)
            {
                Dbg.Log(string.Format("Type {0} doesn't have field {1}", objType.ToString(), fieldName));
                return;
            }
