...
namespace Pit.Sim
{
    ...
    {
        public List<MatchParams> Matches = new List<MatchParams>();
        public int NumDays;


        static Predicate<MatchParams> FindMatch(List<TeamId> teamIds, int round)
        {
            return delegate (MatchParams param)
            {
                return param.TeamIds == teamIds && param.When.Day == round;
            };
        }
        ...
        public void MakeRoundRobinSchedule(int numTeams, int numRounds)
        {
            ...
            //    {
            //        Team tm1 = Sim.League.Teams[t1];
            //        Team tm2 = Sim.League.Teams[t2];

            //        GM_IDList idList = new GM_IDList();
            //        idList.Add(tm1.Id);
            //        idList.Add(tm2.Id);

            //        MatchParams match = new MatchParams();
        ...
        for (int teamIndex1 = 0; teamIndex1 < matchInfo.GetLength(0); teamIndex1++)
        {
            ...
            {
                Team t1 = PT_Game.League.Teams[teamIndex1];
                Team t2 = PT_Game.League.Teams[matchInfo[teamIndex1, round]];
